...
namespace Assets.HeroEditor4D.Common.Scripts.Editor
{
    ...
    {
        private static SpriteCollection[] _collections;

        public static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
        {
            var assets = importedAssets.Union(deletedAssets).Union(movedAssets).ToList();

            if (assets.All(i => !i.Contains("/Sprites/"))) return;

            _collections ??= Resources.LoadAll<SpriteCollection>("");

            foreach (var collection in _collections)
            {
                var folders = collection.SpriteFolders.Select(AssetDatabase.GetAssetPath).ToList();
        ...
        public static void Refresh(SpriteCollection spriteCollection)
        {
            ...
            if (spriteCollection.DebugLogs)
            {
                Debug.Log($"<color=yellow>SpriteCollection [{spriteCollection.Id}] has been refreshed.</color>");
            }
        }
