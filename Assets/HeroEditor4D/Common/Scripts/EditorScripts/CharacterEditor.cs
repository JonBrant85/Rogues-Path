...
namespace Assets.HeroEditor4D.Common.Scripts.EditorScripts
{
    ...
    public class CharacterEditor : MonoBehaviour
    {
        ...
        public void OnSelectTab(bool value)
        {
            ...
            {
                var itemParams = items[i].Params;

                if (items[i].Id != null && !iconIds.Contains(itemParams.IconId))
                {
                    items.RemoveAt(i);
	    ...
	    {
            StartCoroutine(StandaloneFilePicker.OpenFile("Open as JSON", "", ".json", (success, path, bytes) => { if (success) Character.FromJson(Encoding.Default.GetString(bytes), silent: false); }));
	    }
