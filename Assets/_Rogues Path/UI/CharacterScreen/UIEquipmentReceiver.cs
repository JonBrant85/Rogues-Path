...
namespace _Rogues_Path.UI.CharacterScreen {
    public class UIEquipmentReceiver : UIBehaviour, IEventSystemHandler, IDropHandler, IPointerEnterHandler, IPointerExitHandler {
        ...
        public UIEquipmentSlot GetSlotByType(EquipmentPart type) {
            UIEquipmentSlot[] slots = this.m_SlotsContainer.GetComponentsInChildren<UIEquipmentSlot>();
        ...
        public void OnDrop(PointerEventData eventData) {
            ...
            UIEquipmentSlot itemSlot = (UIEquipmentSlot)slotBase;
            ...
            if (itemSlot != null && itemSlot.IsAssigned()) {
                ...
                UIEquipmentSlot equipSlot = this.GetSlotByType(itemSlot.GetEquipmentInfo().EquipType);
        ...
        /// <param name="eventData">Event data.</param>
        private UIEquipmentSlot ExtractSlot(PointerEventData eventData) {
            if (eventData.pointerPress == null)
                return null;

            // Try getting slot base component from the selected object
            UIEquipmentSlot slotBase = eventData.pointerPress.GetComponent<UIEquipmentSlot>();

            // Check if we failed to get a slot from the pressed game object directly
            if (slotBase == null)
                slotBase = eventData.pointerPress.GetComponentInChildren<UIEquipmentSlot>();
